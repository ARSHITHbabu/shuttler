...
class _StudentBatchesScreenState extends ConsumerState<StudentBatchesScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return authState.when(
      data: (authValue) {
        ...
        return _buildContent(studentId);
      },
  ...
  Widget _buildContent(int studentId) {
            ...
            children: [
              ...
              batchesAsync.when(
                ...
                error: (error, stack) => ErrorDisplay(
                  ...
                  onRetry: () => ref.invalidate(studentBatchesProvider(studentId)),
                ),
              ),
...
class _BatchCard extends StatelessWidget {
  ...
  String _formatCoachNames() {
    // First check if we have the new format (assigned_coaches list)
    if (batch.assignedCoaches.isNotEmpty) {
      final names = batch.assignedCoaches.map((coach) => coach.name).where((name) => name.isNotEmpty).toList();
      if (names.isNotEmpty) {
        return names.join(', ');
      }
    }
    ...
    // If we have coach IDs but no names, indicate that coaches are assigned but names unavailable
    if (batch.assignedCoachIds.isNotEmpty) {
      return 'Coaches assigned (${batch.assignedCoachIds.length})';
    }
...
class _BatchStudentsSheet extends ConsumerWidget {
  final Batch batch;
  ...
  Widget _buildStudentsList(WidgetRef ref) {
    ...
    return studentsAsync.when(
      ...
      error: (error, stack) => ErrorDisplay(
        ...
        onRetry: () => ref.invalidate(batchStudentsProvider(batch.id)),
      ),
      data: (students) {
        if (students.isEmpty) {
          return const Center(
